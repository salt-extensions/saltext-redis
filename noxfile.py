...
def tests(session):
    ...
    try:
        ...
        session.run(
            ...
            "--omit=src/saltext/redis/*",
            "--include=tests/*",
        )
        try:
            session.run("coverage", "report", "--show-missing", "--include=src/saltext/redis/*")
            ...
            #    "--include=src/saltext/redis/*,tests/*"
            # )
        finally:
...
def gen_api_docs(session):
    ...
    session.run(
        ...
        "docs/ref/",
        "src/saltext",
        "src/saltext/redis/config/schemas",
    )
